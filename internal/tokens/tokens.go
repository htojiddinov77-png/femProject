...
type Token struct {
	Plaintext string    `json:"token"` // token string that will send the client
	Hash      []byte    `json:"-"` // hashes plaintext for security
	UserID    int       `json:"-"`
	Expiry    time.Time `json:"expiry"`
	Scope     string    `json:"-"` // what token is used for
}
